...
class TestLambda(unittest.TestCase):
    ...
    def test_lambda_handler_stopping_with_cname_and_zone_tags(
            self,
            instance_info,
            hosted_zones,
        ...
        get_subnet_cidr.return_value = '172.31.80.0/20'
        ...
        results = put_item_in_dynamodb_table(client, 'DDNS', 'i-00deb668716374ec7', {'Reservations': [{'Instances': [{'AmiLaunchIndex': 123, 'ImageId': 'string', 'InstanceId': 'i-00deb668716374ec7', 'InstanceType': 't1.micro', 'KeyName': 'string', 'LaunchTime': '2015-01-01T00:00:00', 'Monitoring': {'State': 'enabled'}, 'PrivateDnsName': 'ip-172-31-90-228.ddnslambda.com', 'PrivateIpAddress': '172.31.90.228', 'StateTransitionReason': 'string', 'SubnetId': 'subnet-24fe650a', 'VpcId': 'vpc-43248d39', 'Architecture': 'x86_64', 'Tags': [{'Key': 'CNAME', 'Value': 'machine1.ddnslambda.com.'}]}], 'OwnerId': 'string', 'RequesterId': 'string', 'ReservationId': 'string'}]})
        print('results: '+str(results))

        response = lambda_handler(event, 'context', dynamodb_client=client)
        assert response[0] == 'Successfully removed recordsets'
        assert response[1] == 'Deleted CNAME record in zone id: Z2705FFK9RBG8N for hosted zone machine1.ddnslambda.com. with value: ip-172-31-90-228.ddnslambda.com'
        assert response[2] == 'Deleted A record in zone id: Z2705FFK9RBG8N for hosted zone machine1.ddnslambda.com. with value: 172.31.90.228'
        assert response[3] == 'Deleted PTR record in zone id: Z2705FFK9RBG8O for hosted zone 228.90.31.172.in-addr.arpa with value: machine1.ddnslambda.com.'
