...
class TestLambda(unittest.TestCase):
    ...
    def test_lambda_handler_private_subnet_with_dhcp_with_cname_and_zone_tags(
            self,
            instance_info,
            hosted_zones,
                    ...
                    'Instances': [
                        {
                            ...
                            'PrivateDnsName': 'ip-172-31-90-228.ddnslambda.com',
        ...
        # Because the dns option set contains ddnslambda.com, dhcp will attempt to create A record in

        response = lambda_handler(event, 'context', dynamodb_client=client)
        assert response[0] == 'Successfully created recordsets'
        assert response[1] == 'Created CNAME record in zone id: Z2705FFK9RBG8N for hosted zone machine1.ddnslambda.com. with value: ip-172-31-90-228.ddnslambda.com'
        assert response[2] == 'Created A record in zone id: Z2705FFK9RBG8N for hosted zone machine1.ddnslambda.com. with value: 172.31.90.228'
        assert response[3] == 'Created PTR record in zone id: Z2705FFK9RBG8O for hosted zone 228.90.31.172.in-addr.arpa with value: machine1.ddnslambda.com.'
        mock.stop()
